...
async function GET(request) {
    ...
    if (code) {
        ...
        if (data?.session) {
            ...
            const isValidRole = (v)=>v === "superadmin" || v === "distributor" || v === "end_user";
            if (data.user?.id && isValidRole(roleFromMeta)) {
                const { error: roleUpdateError } = await supabase.from("user_profiles").update({
                    role: roleFromMeta
                }).eq("id", data.user.id);
                if (roleUpdateError) {
    ...
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].redirect(`${origin}/auth/login?error=no_code`);
}
}),
];
